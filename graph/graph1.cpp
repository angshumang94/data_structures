...
void Graph::Dijkstra(int v, int dest){
	...
	sift_up(v);	// Sift up the start vertex value
	...
	while(count!=0){
		...
		for(int i = 0; i< nodes ;i++){
			if(visited[i] == false && adj[curr][i]!=0 && (arr[pos[curr]]->weight + adj[curr][i]) < arr[pos[i]]->weight){
				arr[pos[i]]->weight = arr[pos[curr]]->weight + adj[curr][i];
				arr[pos[i]]->parent = curr;
				sift_up(i);
	...
	if(visited[dest] == true){
		...
		cout << "Path cost : " << arr[pos[dest]]->weight << endl;
		cout << "Path : ";

		while(temp != -1){
			parent = arr[pos[temp]]->parent;
			cout << temp << " <-- ";
			temp = parent;
